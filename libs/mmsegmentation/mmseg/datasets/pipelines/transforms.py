...
@PIPELINES.register_module()
class Normalize_Dynamic(object):
    ...
    def __call__(self, results):
        ...
        img_mean = np.array([img[...,i].mean() for i in range(img.shape[-1])])
        img_std = np.array([img[...,i].std() for i in range(img.shape[-1])])
        results['img'] = mmcv.imnormalize(results['img'], img_mean, img_std,
                                          to_rgb=False)
        results['img_norm_cfg'] = dict(
            mean=img_mean, std=img_std, to_rgb=self.to_rgb)
        return results
