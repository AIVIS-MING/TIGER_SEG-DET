...
DATA_DIR = "/media/mingfan/DataHDD/DATA_Tiger/Seg_new5"
...
WEIGHT_DIR = "/media/mingfan/DATASSD/TIGER/mmsegmentation/WEIGHT/uppernet-Knet-dim_reduce"

config_file = osp.join(WEIGHT_DIR, 'knet_tiger_swin_2048.py')
checkpoint_file = osp.join(WEIGHT_DIR, 'iter_4000.pth')
...
if __name__=='__main__':
    ...
    for i in tqdm(range(0, length, 1)):
        ...
        result = inference_segmentor(model, img)[1]
        seg_l.append(result[0] * ROI_mask)
        gt_l.append(gt)
        #result = result + 1
        # show_preded_result(img, gt, palette=PALETTE_GT, win_name='GT', show=True, wait_time=1, out_file=None, opacity=0.3)
        # show_preded_result(img, SegResult, palette=PALETTE_GT, win_name='pred', show=True, wait_time=0, out_file=None, opacity=0.3)

    IoU = mmseg.core.evaluation.mean_iou(seg_l, gt_l, 3, ignore_index= 255, reduce_zero_label=True)
    f_score = mmseg.core.evaluation.mean_fscore(seg_l, gt_l, 3, ignore_index= 255, reduce_zero_label=True)
    dice_score = mmseg.core.evaluation.mean_dice(seg_l, gt_l, 3, ignore_index= 255, reduce_zero_label=True)
    ...
    result_df = pd.DataFrame({
        ...
        "precision":f_score['Precision'],
        "recall":f_score["Recall"],
        "dice_score":dice_score['Dice']
        }, index=CLASSES_PRED_3)
    
    print("Result Mean:")
    print("aAcc:",IoU['aAcc'])
    print("mIoU:",np.nanmean(result_df['IoU']))
    print("mAcc:",np.nanmean(result_df['Acc']))
    print("mFScore:",np.nanmean(result_df['f_score']))
    print("mDiceScore:",np.nanmean(result_df['dice_score']))
    print("precision:",np.nanmean(result_df['precision']))
    print("recall:",np.nanmean(result_df['recall']))
    print()
    print("class_results:")
    print(result_df)